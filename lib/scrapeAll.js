...
// Used by the scheduler Worker; public schedule requests never scrape.

import { RINKS } from './rinks.js';
import { activitySet } from './activities.js';
import { scrapeDaySmart, createDaySmartJsonLoader } from './scrapers/daysmart.js';
...
export async function scrapeAll({ jitterRecTimes = false, activities } = {}) {
  ...
  const loadJson = createDaySmartJsonLoader();
  const entries = await Promise.all(
    Object.entries(RINKS).map(async ([key, rink]) => {
      try {
        ...
        switch (rink.system) {
          case 'daysmart': {
            const result = await scrapeDaySmart(rink.config, { activities, loadJson });
