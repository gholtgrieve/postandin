...
async function scrapeDaySmartFeed({
  ...
  const [data, rdata] = await Promise.all([
    dsGet(`https://apps.daysmartrecreation.com/dash/jsonapi/api/v1/events?${eventFilter}&company=${company}&filter[start__gte]=${today}&page[size]=${pageSize}&include=homeTeam`, fetchImpl),
    dsGet(`https://apps.daysmartrecreation.com/dash/jsonapi/api/v1/resources?company=${company}`, fetchImpl),
  ]);
  ...
  await Promise.all(leagueIds.map(async id => {
    try {
      const ld = await dsGet(`https://apps.daysmartrecreation.com/dash/jsonapi/api/v1/leagues/${id}?company=${company}`, fetchImpl);
