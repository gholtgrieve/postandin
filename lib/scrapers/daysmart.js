...
// Drop-in Hockey and Public Skate by passing them in `activities`. The hockey
// and Public Skate feeds are fetched independently so callers can preserve
// last-known-good data for only the activity that failed.
...
export async function scrapeDaySmart(
  ...
  { activities, fetchImpl = fetch, loadJson = createDaySmartJsonLoader(fetchImpl) } = {},
) {
  ...
  const hockeyActivities = [ACTIVITY_STICK_AND_PUCK, ACTIVITY_DROP_IN_HOCKEY]
    .filter(activity => requested.has(activity));
  if (hockeyActivities.length) {
    attempted.push(...hockeyActivities);
    try {
      sessions.push(...await scrapeDaySmartFeed({
        company,
        eventFilter: `filter[homeTeam.sport_id__in]=${sportId}`,
        resourceIds,
        activities: hockeyActivities,
        pageSize: 200,
        today,
        loadJson,
      }));
    } catch (error) {
      console.error(`DaySmart ${company} hockey feed failed:`, error.message, error.stack);
      failures.push(...hockeyActivities);
    }
  }

  if (requested.has(ACTIVITY_PUBLIC_SKATE)) {
    attempted.push(ACTIVITY_PUBLIC_SKATE);
    try {
      const eventFilter = company === 'snoking'
      ...
      console.error(`DaySmart ${company} Public Skate feed failed:`, error.message, error.stack);
      failures.push(ACTIVITY_PUBLIC_SKATE);
    }
  }

  return { sessions, attempted, failures };
}

async function scrapeDaySmartFeed({
  company,
  eventFilter,
  resourceIds,
  activities,
  ...
  const [data, rdata] = await Promise.all([
    loadJson(`https://apps.daysmartrecreation.com/dash/jsonapi/api/v1/events?${eventFilter}&company=${company}&filter[start__gte]=${today}&page[size]=${pageSize}&include=homeTeam`),
    loadJson(`https://apps.daysmartrecreation.com/dash/jsonapi/api/v1/resources?company=${company}`),
  ]);
  ...
  await Promise.all(leagueIds.map(async id => {
    try {
      const ld = await loadJson(`https://apps.daysmartrecreation.com/dash/jsonapi/api/v1/leagues/${id}?company=${company}`);
      leagueMap[id] = ld?.data?.attributes?.name ?? null;
    } catch { leagueMap[id] = null; }
  }));

  // Some DaySmart programs publish separate skater and goalie registrations
  // with blank event descriptions. Request the related home-team records in
  // the main event response so the Worker does not rely on extra metadata
  // requests that can be throttled independently.
  const teamMap = includedTeamMap(data?.included);
...
export function includedTeamMap(included) {
  return Object.fromEntries(
    (included ?? [])
      .filter(item => item?.type === 'teams' && item.id)
      .map(item => [String(item.id), item.attributes?.name ?? null])
  );
}
    ...
    .map(ev => {
      ...
      const details = activity === ACTIVITY_DROP_IN_HOCKEY
        ...
        : activity === ACTIVITY_PUBLIC_SKATE
          ? { subtitle: null, eligibility: eligibility() }
          : { subtitle: stickSubtitle, eligibility: eligibility() };
      const startDate = start ? new Date(start).toISOString().slice(0, 10) : null;
      const role = activity === ACTIVITY_DROP_IN_HOCKEY
        ? registrationRole(a.desc, teamMap[String(a.hteam_id)])
        : registrationRole(a.desc);
      const sourceLabel = leagueName ?? firstLabel(a) ?? {
        [ACTIVITY_STICK_AND_PUCK]: 'Stick & Puck',
        [ACTIVITY_DROP_IN_HOCKEY]: 'Drop-in Hockey',
        [ACTIVITY_PUBLIC_SKATE]: 'Public Skate',
      }[activity];
      return {
        id:      ev.id,
        start:   start ?? null,
        ...
        title:   activity === ACTIVITY_STICK_AND_PUCK
          ...
          : activity === ACTIVITY_PUBLIC_SKATE ? 'Public Skate' : sourceLabel,
        ...
        _daySmart: {
          ...
          role,
          roleBase: roleBase(a.desc) ?? (role ? sourceLabel.toLowerCase() : null),
        },
      };
    })
    .filter(s => !!s.start);
...
// One loader per scrape: share parsed data and failures across rink/feed calls.
// Never persist this map globally; a later scrape must retry and fetch fresh data.
export function createDaySmartJsonLoader(fetchImpl = fetch) {
  const pending = new Map();
  return url => {
    if (!pending.has(url)) pending.set(url, dsGet(url, fetchImpl));
    return pending.get(url);
  };
}

async function dsGet(url, fetchImpl) {
  let lastError;
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      lastError = error;
      if (attempt === 0) await sleep(150 + Math.random() * 200);
    }
  }
  throw lastError;
}
...
function registrationRole(desc, teamName) {
  const match = String(desc ?? '').match(/^\s*(Skater|Goalie)\s*-\s*/i);
  if (match) return match[1].toLowerCase();
  const teamMatch = String(teamName ?? '').match(/\b(Skater|Goalie)\s*$/i);
  return teamMatch ? teamMatch[1].toLowerCase() : null;
}
