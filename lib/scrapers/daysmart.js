...
export async function scrapeDaySmart(
  ...
  // with blank event descriptions. Only fetch team names for roleless records
  // that share an otherwise-identical Drop-in session, keeping the extra
  // requests narrow and avoiding capacity-based role guesses.
  const dropInGroups = new Map();
  for (const ev of data?.data ?? []) {
    const a = ev.attributes ?? {};
    const leagueName = leagueMap[a.league_id] ?? null;
    if (classifyDaySmartActivity({ company, leagueName, eventText: eventText(a) }) !== ACTIVITY_DROP_IN_HOCKEY) continue;
    const key = [a.league_id, a.resource_id, a.start, a.end].join('|');
    if (!dropInGroups.has(key)) dropInGroups.set(key, []);
    dropInGroups.get(key).push(ev);
  }
  const teamIds = [...new Set(
    [...dropInGroups.values()]
      .filter(events => events.length > 1)
      .flatMap(events => events)
      .filter(ev => !registrationRole(ev.attributes?.desc) && ev.attributes?.hteam_id)
      .map(ev => String(ev.attributes.hteam_id))
  )];
  const teamMap = {};
  await Promise.all(teamIds.map(async id => {
    try {
      const team = await dsGet(`https://apps.daysmartrecreation.com/dash/jsonapi/api/v1/teams/${id}?company=${company}`);
      teamMap[id] = team?.data?.attributes?.name ?? null;
    } catch { teamMap[id] = null; }
  }));
