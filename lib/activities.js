...
export const RECTIMES_PUBLIC_SKATE_LABELS = Object.freeze({
  1145: Object.freeze([]),
  1146: Object.freeze(['Public Skate']),
});

// Shared across RecTimes venues: if another configured rink publishes this
// reviewed label, it receives the same normalized audience classification.
export const RECTIMES_FEMALE_STICK_AND_PUCK_LABELS = Object.freeze([
  'Female Stick & Puck',
]);
...
export const EVERETT_PUBLIC_SKATE_LABELS = Object.freeze([
  'Public Session',
  'Public Skate',
  // Confirmed directly with the rink on 2026-08-18. The source may attach
  // Hockey sport metadata to this title incorrectly; classify by the reviewed
  // title and continue to ignore that broad metadata tag.
...
export function classifyRecTimesActivity(venueId, groupName) {
  if (RECTIMES_PUBLIC_SKATE_LABELS[venueId]?.includes(groupName)) {
    return ACTIVITY_PUBLIC_SKATE;
  }
...
export function classifyEverettActivity(title) {
  if (EVERETT_PUBLIC_SKATE_LABELS.includes(title)) return ACTIVITY_PUBLIC_SKATE;
...
export function recTimesStickDetails(groupName) {
  if (RECTIMES_FEMALE_STICK_AND_PUCK_LABELS.includes(groupName)) {
    return {
      subtitle: 'Female',
      eligibility: eligibility({ audience: 'female' }),
    };
  }
  return { subtitle: null, eligibility: eligibility() };
}
