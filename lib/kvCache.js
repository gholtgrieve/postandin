...
const coldByNamespace = new WeakMap();
...
export async function readThrough(kv, key, freshMs, staleTtlS, fetchFresh, waitUntil) {
  ...
  // Cold: no usable entry. Share one fetch-and-store promise within this isolate
  // so concurrent misses cannot multiply upstream calls.
  let cold = coldByNamespace.get(kv);
  if (!cold) coldByNamespace.set(kv, cold = new Map());
  if (!cold.has(key)) {
    const task = (async () => {
      const data = await fetchFresh();
      const write = kv.put(
        ...
        { expirationTtl: staleTtlS },
      ).catch(e => console.error('kvCache cold write failed', key, e.message, e.stack));
